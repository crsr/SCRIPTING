...
if ( !$logfile ) {
    $logfile = "parser_T47_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T47_PLAYLIST_IANUARIE_2012.XLS";
					...
					if( ! defined $channel_founded) { $channel_founded[0] = "null"; }
						...
						my $workbook = $parser->parse(realpath($file)); # parse file
								...
								my($data_sheet, $sheet_name); # set variables for parsing
										...
										$sheet_name = undiacritic($data_sheet->{Name}); 
												...
												for my $row (0 .. $data_sheet->{MaxRow}) {
													if($row != 0){
														...
														#my $c6 = $data_sheet->get_cell($row, 13);#secunde
														#my $c7 = $data_sheet->get_cell($row, 11);#nr difuzari
														...
														#$stmt->execute( clean_string($c1->value()), "null", clean_string($c5->value()), "null", clean_string($c3string), clean_string($c4->value()), "T47", $month_founded[0], $year_founded[0], $channel_founded[0], "-");			
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => clean_string($c1->value()), "EMISIUNE" => "null", "MINUTE" => clean_string($c5->value()), "SECUNDE" => "null", "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T47"});
													}
												}									
									} 
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T47_'.$file);
