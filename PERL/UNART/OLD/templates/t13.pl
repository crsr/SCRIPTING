...
if ( !$logfile ) {
    $logfile = "parser_T13_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T13_PLAYLIST_IANUARIE_2012.XLS";
					...
					if( ! defined $channel_founded) { $channel_founded[0] = "null"; }
						...
						my $workbook = $parser->parse(realpath($file)); # parse file
								...
								my($data_sheet, $sheet_name); # set variables for parsing
										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (0 .. $data_sheet->{MaxRow}) {
													if($row != 0){
														...
														my $c3 = $data_sheet->get_cell($row, 4); next unless $c3; #opera
														my $c4 = $data_sheet->get_cell($row, 5); next unless $c4; #artist
														my $c5 = $data_sheet->get_cell($row, 2); next unless $c5; #minute
														my $c6 = $data_sheet->get_cell($row, 3); next unless $c6; #secunde
														#my $c7 = $data_sheet->get_cell($row, 8);#nr difuzari
														...
														$c3string =~ s/"//;
														$c3string =~ s/.rr//;
														if($c3->value() eq "" and $c4->value() eq "" and $c5->value() eq "" and $c6->value() eq "") { next; } #remove empty data
														
														#$stmt->execute( "null", "null", clean_string($c5->value()), clean_string($c6->value()), clean_string($c3string), clean_string($c4->value()), "T13", $month_founded[0], $year_founded[0], $channel_founded[0], "null"	);	
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => "null", "EMISIUNE" => "null", "MINUTE" => clean_string($c5->value()), "SECUNDE" => clean_string($c6->value()), "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T13"});			
													}
												}									
									} 
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T13_'.$file);
