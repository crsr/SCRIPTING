...
if ( !$logfile ) {
    $logfile = "parser_T3_".$log_file_data.".log";
									...
									foreach my $data_sheet (@{$workbook->{Worksheet}}) {
										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (0 .. $data_sheet->{MaxRow}) {
													if($row != 0){
														#my $c1 = $data_sheet->get_cell($row, 1); # data
														my $c2 = $data_sheet->get_cell($row, 0); next unless $c2; # emisiune
														my $c3 = $data_sheet->get_cell($row, 1); next unless $c3; # opera
														my $c4 = $data_sheet->get_cell($row, 2); next unless $c4; # interpret
														my $c7 = $data_sheet->get_cell($row, 4); next unless $c7; # nr difuzari
														my $c8 = $data_sheet->get_cell($row, 5); next unless $c8; # min
														my $c9 = $data_sheet->get_cell($row, 6); next unless $c9; # sec
														...
														$c3string =~ s/"//;
														$c3string =~ s/.rr//;
														if($c2->value() eq "" and $c3->value() eq "" and $c4->value() eq "" and $c8->value() eq "" and $c7->value() eq "" and $c9->value() eq "") { next; } #remove empty data
														
														#$stmt->execute( "null", clean_string($c2->value()), clean_string($c8->value()), clean_string($c9->value()), clean_string($c3string), clean_string($c4->value()), "T3", $month_founded[0], $year_founded[0], $channel_founded[0], clean_string($c7->value()));		
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => "null", "EMISIUNE" => clean_string($c2->value()), "MINUTE" => clean_string($c8->value()), "SECUNDE" => clean_string($c9->value()), "OPERA" => $c3string, "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => clean_string($c7->value()), "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T3"});															
													}
												}									
									} 
