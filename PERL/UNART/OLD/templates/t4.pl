...
if ( !$logfile ) {
    $logfile = "parser_T4_".$log_file_data.".log";
										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (2 .. $data_sheet->{MaxRow}) {
													if($row != 2){
														...
														#my $c2 = $data_sheet->get_cell($row, 0); # emisiune
														...
														my $c8 = $data_sheet->get_cell($row, 0); next unless $c8; # min
														my $c9 = $data_sheet->get_cell($row, 1); next unless $c9; # sec
														...
														$c3string =~ s/"//;
														$c3string =~ s/.rr//;
														if($c3->value() eq "" and $c4->value() eq "" and $c8->value() eq "" and $c7->value() eq "" and $c9->value() eq "") { next; } #remove empty data
														
														#$stmt->execute( "null", "null", clean_string($c8->value()), clean_string($c9->value()), clean_string($c3string), clean_string($c4->value()), "T4", $month_founded[0], $year_founded[0], $channel_founded[0], clean_string($c7->value()));								
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => "null", "EMISIUNE" => "null", "MINUTE" => clean_string($c8->value()), "SECUNDE" => clean_string($c9->value()), "OPERA" => $c3string, "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => clean_string($c7->value()), "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T4"});															
													}
												}									
									} 
