...
if ( !$logfile ) {
    $logfile = "parser_T8_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T8_PLAYLIST_IANUARIE_2012.XLS";
										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (0 .. $data_sheet->{MaxRow}) {
													if($row != 0){
														...
														my $c5 = $data_sheet->get_cell($row, 9); next unless $c5; #minute
														my $c6 = $data_sheet->get_cell($row, 10); next unless $c6; #secunde
														...
														$c3string =~ s/"//;
														$c3string =~ s/.rr//;
														if($c2->value() eq "" and $c3->value() eq "" and $c4->value() eq "" and $c5->value() eq "" and $c6->value() eq "" and $c7->value() eq "") { next; } #remove empty data
														
														#$stmt->execute( "null", clean_string($c2->value()), clean_string($c5->value()), clean_string($c6->value()), clean_string($c3string), clean_string($c4->value()), "T8", $month_founded[0], $year_founded[0], $channel_founded[0], clean_string($c7->value())	);
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => "null", "EMISIUNE" => clean_string($c2->value()), "MINUTE" => clean_string($c5->value()), "SECUNDE" => clean_string($c6->value()), "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => clean_string($c7->value()), "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T8"});		
													}
												}									
									} 
