...
if ( !$logfile ) {
    $logfile = "parser_T39_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T39_PLAYLIST_IANUARIE_2012.XLS";
					...
					if( ! defined $channel_founded) { $channel_founded[0] = "null"; }
						...
						my $workbook = $parser->parse(realpath($file)); # parse file
								...
								my($data_sheet, $sheet_name); # set variables for parsing
									foreach my $data_sheet (@{$workbook->{Worksheet}}) {
										...
										$sheet_name = undiacritic($data_sheet->{Name}); 
												...
												for my $row (7 .. $data_sheet->{MaxRow}) {
													if($row != 8){
														my $c1 = $data_sheet->get_cell($row, 3); next unless $c1; #data
														#my $c2 = $data_sheet->get_cell($row, 2);#emisiune
														my $c3 = $data_sheet->get_cell($row, 12); next unless $c3; #opera
														my $c4 = $data_sheet->get_cell($row, 21); next unless $c4; #artist
														...
														#$stmt->execute( clean_string($c1->value()), "null", clean_string($c5->value()), clean_string($c6->value()), clean_string($c3string), clean_string($c4->value()), "T39", $month_founded[0], $year_founded[0], $channel_founded[0], clean_string($c7->value()));	
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => clean_string($c1->value()), "EMISIUNE" => "null", "MINUTE" => clean_string($c5->value()), "SECUNDE" => clean_string($c6->value()), "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => clean_string($c7->value()), "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T39"});	
													}
												}									
									} 
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T39_'.$file);
