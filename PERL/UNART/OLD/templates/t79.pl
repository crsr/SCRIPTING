...
if ( !$logfile ) {
    $logfile = "parser_T79_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T79_PLAYLIST_IANUARIE_2012.XLS";
								...
								my($data_sheet, $sheet_name); # set variables for parsing
									foreach my $data_sheet (@{$workbook->{Worksheet}}) {
										...
										$sheet_name = undiacritic($data_sheet->{Name}); 
												...
												for my $row (4 .. $data_sheet->{MaxRow}) {
													if($row != 4 and $row != 5){
														...
														my $c4 = $data_sheet->get_cell($row, 3); next unless $c4; #artist
														my $c5 = $data_sheet->get_cell($row, 5); next unless $c5; #minute
														#my $c6 = $data_sheet->get_cell($row, 3);#secunde
														my $c7 = $data_sheet->get_cell($row, 6); next unless $c7; #nr difuzari
														...
														$c3string =~ s/.rr//;
														$c3string =~ s/~/'/;
														if($c3->value() eq "" and $c4->value() eq "" and $c5->value() eq "" and $c7->value() eq "") { next; } #remove empty data
														
														#$stmt->execute( "null", "null", clean_string($c5->value()), "null", clean_string($c3string), clean_string($c4->value()), "T79", $month_founded[0], $year_founded[0], $channel_founded[0], clean_string($c7->value()));		
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => "null", "EMISIUNE" => "null", "MINUTE" => clean_string($c5->value()), "SECUNDE" => "null", "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => clean_string($c7->value()), "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T79"});
													}
												}									
									} 
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T79_'.$file);
