...
if ( !$logfile ) {
    $logfile = "parser_T55_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T55_PLAYLIST_IANUARIE_2012.XLS";
					...
					if( ! defined $channel_founded) { $channel_founded[0] = "null"; }
						...
						my $workbook = $parser->parse(realpath($file)); # parse file
								...
								my($data_sheet, $sheet_name); # set variables for parsing
									foreach my $data_sheet (@{$workbook->{Worksheet}}) {
										...
										$sheet_name = undiacritic($data_sheet->{Name}); 
												...
												for my $row (6 .. $data_sheet->{MaxRow}) {
													if($row != 6 and $row != 7){
														my $c1 = $data_sheet->get_cell($row, 0); next unless $c1; #data zi
														my $c1a = $data_sheet->get_cell($row, 2); next unless $c1a; #data luna
														my $c1b = $data_sheet->get_cell($row, 3); next unless $c1d; #data an
														#my $c2 = $data_sheet->get_cell($row, 1);#emisiune
														my $c3 = $data_sheet->get_cell($row, 11); next unless $c3; #opera
														my $c4 = $data_sheet->get_cell($row, 15); next unless $c4; #artist
														...
														my $date = $c1->value().'-'.$c1a->value().'-'.$c1b->value();
														...
														$c3string =~ s/.rr//;
														$c3string =~ s/~/'/;
														if($c1->value() eq "" and $c1a->value() eq "" and $c1b->value() eq "" and $c3->value() eq "" and $c4->value() eq "" and $c5->value() eq "" and $c6->value() eq "") { next; } #remove empty data
														
														#$stmt->execute( clean_string($date), "null", clean_string($c5->value()), clean_string($c6->value()), clean_string($c3string), clean_string($c4->value()), "T55", $month_founded[0], $year_founded[0], $channel_founded[0], "-");
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => clean_string($c1->value()), "EMISIUNE" => "null", "MINUTE" => clean_string($c5->value()), "SECUNDE" => clean_string($c6->value()), "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T55"});
													}
												}									
									} 
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T55_'.$file);
