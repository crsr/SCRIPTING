...
if ( !$logfile ) {
    $logfile = "parser_T50_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T50_PLAYLIST_IANUARIE_2012.XLS";
					...
					if( ! defined $channel_founded) { $channel_founded[0] = "null"; }
						...
						my $workbook = $parser->parse(realpath($file)); # parse file
								...
								my($data_sheet, $sheet_name); # set variables for parsing
										...
										$sheet_name = undiacritic($data_sheet->{Name}); 
												...
												for my $row (0 .. $data_sheet->{MaxRow}) {
													if($row != 0){
														...
														my $c3 = $data_sheet->get_cell($row, 0); next unless $c3; #opera
														...
														#$stmt->execute( "null", "null", "null", clean_string($c6->value()), clean_string($c3string), clean_string($c4->value()), "T50", $month_founded[0], $year_founded[0], $channel_founded[0], clean_string($c7->value()));	
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => "null", "EMISIUNE" => "null", "MINUTE" => "null", "SECUNDE" => clean_string($c6->value()), "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => clean_string($c7->value()), "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T50"});
													}
												}									
									} 
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T50_'.$file);
