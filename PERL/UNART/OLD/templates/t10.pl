...
if ( !$logfile ) {
    $logfile = "parser_T10_".$log_file_data.".log";
								...
								my($data_sheet, $sheet_name); # set variables for parsing
									foreach my $data_sheet (@{$workbook->{Worksheet}}) {
										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (3 .. $data_sheet->{MaxRow}) {
													if($row != 3){
														...
														my $c4 = $data_sheet->get_cell($row, 8); next unless $c4; #artist
														...
														#$stmt->execute( clean_string($c1->value()), "null", "null", clean_string($c6->value()), clean_string($c3string), clean_string($c4->value()), "T10", $month_founded[0], $year_founded[0], $channel_founded[0], "-"	);	
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => clean_string($c1->value()), "EMISIUNE" => "null", "MINUTE" => "null", "SECUNDE" => clean_string($c6->value()), "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T10"});														
													}
												}									
									} 
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T10_'.$file);
