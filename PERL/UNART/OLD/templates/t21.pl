...
if ( !$logfile ) {
    $logfile = "parser_T21_".$log_file_data.".log";
					...
					if($extension eq ".XLS"){
						...
						my $workbook = $parser->parse(realpath($file)); # parse file
								...
								my($count_row, $count_column, $data_sheet, $cell, $sheet_name); # set variables for parsing
													...
													defined $data_sheet->{MaxCol} && $count_column <= $data_sheet->{MaxCol} ; $count_column++) {
														...
														if($cell) {
															...
															if($cell->Value ne "") {
																...
																# insert into  mongoDB															
																my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "FISIER" => $file, "SHEET" => $sheet_name, "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "RAND" => $count_row, "COLOANA" => $count_column, "VALOARE" => $cell_value, "TIP" => $cell_type, "TEMPLATE" => "T21"});
															}	
														}
													}
											}								
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T21_'.$file);
