...
if ( !$logfile ) {
    $logfile = "parser_T101_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T101_PLAYLIST_IANUARIE_2012.XLS";
					...
					if( ! defined $channel_founded) { $channel_founded[0] = "null"; }
						...
						my $workbook = $parser->parse(realpath($file)); # parse file
								...
								my($data_sheet, $sheet_name); # set variables for parsing
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T101_'.$file);
