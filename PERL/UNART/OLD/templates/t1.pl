...
if ( !$logfile ) {
    $logfile = "parser_T1_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T1_PLAYLIST_IANUARIE_2012.XLS";
									...
									foreach my $data_sheet (@{$workbook->{Worksheet}}) {
										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (0 .. $data_sheet->{MaxRow}) {
													if($row != 0){
														my $c1 = $data_sheet->get_cell($row, 1); next unless $c1;
														my $c2 = $data_sheet->get_cell($row, 2); next unless $c2;
														my $c3 = $data_sheet->get_cell($row, 3); next unless $c3;
														my $c4 = $data_sheet->get_cell($row, 4); next unless $c4;
														my $c5 = $data_sheet->get_cell($row, 5); next unless $c5;
														my $c7 = $data_sheet->get_cell($row, 7); next unless $c7;
														my $c5string = $c5->value();
														$c5string =~ s/.mpg//;
														$c5string =~ s/. mpg//;
														$c5string =~ s/, mpg//;
														$c5string =~ s/.rnpg//;
														...
														$c5string =~ s/"//;
														$c5string =~ s/.rr//;
														if($c1->value() eq "" and $c2->value() eq "" and $c3->value() eq "" and $c4->value() eq "" and $c5->value() eq "" and $c7->value() eq "") { next; } #remove empty data
														
														#$stmt->execute( clean_string($c1->value()), clean_string($c2->value()), clean_string($c3->value()), clean_string($c4->value()), clean_string($c5string), clean_string($c7->value()), "T1", $month_founded[0], $year_founded[0], $channel_founded[0], "-"	);
														
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => clean_string($c1->value()), "EMISIUNE" => clean_string($c2->value()), "MINUTE" => clean_string($c3->value()), "SECUNDE" => clean_string($c4->value()), "OPERA" => $c5string, "ARTIST" => clean_string($c7->value()), "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T1"});														
													}
												}									
									} 
