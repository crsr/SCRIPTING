								...
								my($data_sheet, $sheet_name); # set variables for parsing
									foreach my $data_sheet (@{$workbook->{Worksheet}}) {
										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												my $stmt = $pg_connection->prepare("INSERT INTO first_buffer2 (data_difuzare,emisiune,minute,secunde,opera,artist,template,luna,an,post) VALUES(?,?,?,?,?,?,?,?,?,?)");
												for my $row (0 .. $data_sheet->{MaxRow}) {
													if($row != 0){
														my $c1 = $data_sheet->get_cell($row, 1);
														my $c2 = $data_sheet->get_cell($row, 2);
														my $c3 = $data_sheet->get_cell($row, 3);
														my $c4 = $data_sheet->get_cell($row, 4);
														my $c5 = $data_sheet->get_cell($row, 5);
														my $c7 = $data_sheet->get_cell($row, 7);
