										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (0 .. $data_sheet->{MaxRow}) {
													if($row != 0){
														...
														my $insert = $mango->db($mongo_database)->collection($mongo_collection)->insert({ "DATA_DIFUZARE" => $month_founded[0], "EMISIUNE" => "null", "MINUTE" => clean_string($c5->value()), "SECUNDE" => clean_string($c6->value()), "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "STATUS" => "0", "TOTAL" => $total});	
													}
												}											
									} 
