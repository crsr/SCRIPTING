...
#
#
#	T12CSV template script (IMPORT V2)
# 	DO:
# 	1. Match template
# 	2. Parse file
# 	3. Import mongoDB
					        ...
					        $row += 1; 
					        	if($row > 6){ 	    
						            if ($csv->parse($_)) {
						                ...
						                my $insert = $mango->db($mongo_database)->collection($mongo_collection)->insert({ "DATA_DIFUZARE" => "null", "EMISIUNE" => $channel_founded[0], "MINUTE" => $splitted[2], "SECUNDE" => $splitted[3], "OPERA" => $splitted[4], "ARTIST" => $splitted[6], "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "STATUS" => "0", "TOTAL" => $total});
						            } else {
