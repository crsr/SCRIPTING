...
if ( !$logfile ) {
    $logfile = "parser_T5_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T5_PLAYLIST_IANUARIE_2012.XLS";
								...
								my($data_sheet, $sheet_name); # set variables for parsing
									foreach my $data_sheet (@{$workbook->{Worksheet}}) {
										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (9 .. $data_sheet->{MaxRow}) {
													if($row != 9){
														...
														my $c5 = $data_sheet->get_cell($row, 13); next unless $c5; #minute
														my $c6 = $data_sheet->get_cell($row, 14); next unless $c6; #secunde
														my $c7 = $data_sheet->get_cell($row, 12); next unless $c7; #nr difuzari
														...
														$c3string =~ s/"//;
														$c3string =~ s/.rr//;
														my $a1 = clean_string($c1->value());
														my $a2 = clean_string($c2->value());
														my $a4 = clean_string($c4->value());
														my $a5 = clean_string($c5->value());
														my $a6 = clean_string($c6->value());
														my $a7 = clean_string($c7->value());
														if($a1 eq "" and $a2 eq "" and $c3string eq "" and $a4 eq "" and $a5 eq "" and $a6 eq "" and $a7 eq "") { next; } #remove empty data
														
														#$stmt->execute( clean_string($c1->value()), clean_string($c2->value()), clean_string($c5->value()), clean_string($c6->value()), clean_string($c3string), clean_string($c4->value()), "T5", $month_founded[0], $year_founded[0], $channel_founded[0], clean_string($c7->value()));		
														
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => $a1, "EMISIUNE" => $a2, "MINUTE" => $a5, "SECUNDE" => $a6, "OPERA" => $c3string, "ARTIST" => $a4, "NR_DIFUZARI" => $a7, "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T5"});			
													}
												}									
									} 
