...
if ( !$logfile ) {
    $logfile = "parser_T32_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T32_PLAYLIST_IANUARIE_2012.XLS";
					...
					if( ! defined $channel_founded) { $channel_founded[0] = "null"; }
						...
						my $workbook = $parser->parse(realpath($file)); # parse file
								...
								my($data_sheet, $sheet_name); # set variables for parsing
										...
										$sheet_name = undiacritic($data_sheet->{Name}); 
												...
												for my $row (0 .. $data_sheet->{MaxRow}) {
													if($row != 0){
														...
														my $c2 = $data_sheet->get_cell($row, 8); next unless $c2; #emisiune
														...
														$c3string =~ s/"//;
														$c3string =~ s/.rr//;
														if($c1->value() eq "" and $c3->value() eq "" and $c6->value() eq "") { next; } #remove empty data
														
														#$stmt->execute( clean_string($c1->value()), clean_string($c2->value()), "null", clean_string($c6->value()), clean_string($c3string), "null", "T32", $month_founded[0], $year_founded[0], $channel_founded[0], "-");			
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => clean_string($c1->value()), "EMISIUNE" => clean_string($c2->value()), "MINUTE" => "null", "SECUNDE" => clean_string($c6->value()), "OPERA" => clean_string($c3string), "ARTIST" => "null", "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T32"});		
													}
												}									
									} 
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T32_'.$file);
