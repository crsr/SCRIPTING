...
#
#
#	T7CSV template script (IMPORT V2)
# 	DO:
# 	1. Match template
# 	2. Parse file
# 	3. Import mongoDB
					        ...
					        $row += 1; 
					        	if($row > 9){ 	    
						            if ($csv->parse($_)) {
						                ...
						                my $total = (($splitted[2] * 60) + $splitted[3]) * $splitted[7];

						                my $correct_data = $year_founded_in_condition_for_data[0] . '-' . $month_founded[0] . '-' . $splitted[0];

						                 my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => $correct_data, "EMISIUNE" => $channel_founded[0], "MINUTE" => $splitted[2], "SECUNDE" => $splitted[3], "OPERA" => $splitted[5], "ARTIST" => $splitted[4], "NR_DIFUZARI" => $splitted[7], "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "STATUS" => "0", "TOTAL" => $total});
