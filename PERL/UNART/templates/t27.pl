...
if ( !$logfile ) {
    $logfile = "parser_T27_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T27_PLAYLIST_IANUARIE_2012.XLS";
					...
					if( ! defined $channel_founded) { $channel_founded[0] = "null"; }
						...
						my $workbook = $parser->parse(realpath($file)); # parse file
								...
								my($data_sheet, $sheet_name); # set variables for parsing
										...
										$sheet_name = undiacritic($data_sheet->{Name}); 
												...
												for my $row (1 .. $data_sheet->{MaxRow}) {
													if($row != 1){
														...
														#my $c2 = $data_sheet->get_cell($row, 5);#emisiune
														my $c3 = $data_sheet->get_cell($row, 2); next unless $c3; #opera
														#my $c4 = $data_sheet->get_cell($row, 7);#artist
														my $c5 = $data_sheet->get_cell($row, 4); next unless $c5; #minute
														...
														$c3string =~ s/"//;
														$c3string =~ s/.rr//;
														if($c1->value() eq "" and $c3->value() eq "" and $c5->value() eq "") { next; } #remove empty data
														
														#$stmt->execute( clean_string($c1->value()), "null", clean_string($c5->value()), "null", clean_string($c3string), "null", "T27", $month_founded[0], $year_founded[0], $channel_founded[0], "-");	
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => clean_string($c1->value()), "EMISIUNE" => "null", "MINUTE" => clean_string($c5->value()), "SECUNDE" => "null", "OPERA" => clean_string($c3string), "ARTIST" => "null", "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T27"});														
													}
												}									
									} 
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T27_'.$file);
