										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (1 .. $data_sheet->{MaxRow}) {
													if($row != 1){
														...
														my $correct_data = $year_founded_in_condition_for_data[0] . '-' . $data[1] . '-' . @data[0];

														my $total = (($c5->value() * 60));

														my $insert = $mango->db($mongo_database)->collection($mongo_collection)->insert({ "DATA_DIFUZARE" => clean_string($correct_data), "EMISIUNE" => "null", "MINUTE" => clean_string($c5->value()), "SECUNDE" => "null", "OPERA" => clean_string($c3string), "ARTIST" => "null", "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "STATUS" => "0", "TOTAL" => $total});														
													}
												}							
									} 
