										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (0 .. $data_sheet->{MaxRow}) {
													if($row != 0){
														...
														my $total = (($c3->value() * 60) + $c4->value());

														my $insert = $mango->db($mongo_database)->collection($mongo_collection)->insert({ "DATA_DIFUZARE" => clean_string($c1->value()), "EMISIUNE" => clean_string($c2->value()), "MINUTE" => clean_string($c3->value()), "SECUNDE" => clean_string($c4->value()), "OPERA" => $c5string, "ARTIST" => clean_string($c7->value()), "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "TOTAL" -> $total, "STATUS" => "0"});														
													}
												}											
									} 
