...
if ( !$logfile ) {
    $logfile = "parser_T94_".$log_file_data.".log";
...
my $file = $ARGV[0];
#my $file = "/var/www/html/IMPORT/ALEXANDRA_TOUR_SRL_HUNEDOARA_TV/TRIM_1/T94_PLAYLIST_IANUARIE_2012.XLS";
								...
								my($data_sheet, $sheet_name); # set variables for parsing
									foreach my $data_sheet (@{$workbook->{Worksheet}}) {
										...
										$sheet_name = undiacritic($data_sheet->{Name}); 
												...
												for my $row (10 .. $data_sheet->{MaxRow}) {
													if($row != 10){
														...
														my $c4 = $data_sheet->get_cell($row, 4); next unless $c4; #artist
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T94_'.$file);
