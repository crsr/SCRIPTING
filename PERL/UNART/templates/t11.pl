...
if ( !$logfile ) {
    $logfile = "parser_T11_".$log_file_data.".log";
					...
					if( ! defined $channel_founded) { $channel_founded[0] = "null"; }
						...
						my $workbook = $parser->parse(realpath($file)); # parse file
								...
								my($data_sheet, $sheet_name); # set variables for parsing
										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (5 .. $data_sheet->{MaxRow}) {
													if($row != 5){
														...
														#$stmt->execute( clean_string($c1->value()), "null", "null", "null", clean_string($c3string), "null", "T11", $month_founded[0], $year_founded[0], $channel_founded[0], "-"	);	
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => clean_string($c1->value()), "EMISIUNE" => "null", "MINUTE" => "null", "SECUNDE" => "null", "OPERA" => clean_string($c3string), "ARTIST" => "null", "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => "T11"});		
													}
												}									
									} 
									...
									chomp($file);
									my $old_path = abs_path($file);
									my $new_path = abs_path('T11_'.$file);
