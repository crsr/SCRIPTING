								...
								my($data_sheet, $sheet_name); # set variables for parsing
									foreach my $data_sheet (@{$workbook->{Worksheet}}) {
										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (5 .. $data_sheet->{MaxRow}) {
													if($row != 5){
														my $c1 = $data_sheet->get_cell($row, 4); next unless $c1; #data
														#my $c2 = $data_sheet->get_cell($row, 1);#emisiune
														my $c3 = $data_sheet->get_cell($row, 1); next unless $c3; #opera
														#my $c4 = $data_sheet->get_cell($row, 8);#artist
														#my $c5 = $data_sheet->get_cell($row, 2);#minute
														#my $c6 = $data_sheet->get_cell($row, 3);#secunde
														#my $c7 = $data_sheet->get_cell($row, 6);#nr difuzari
														my $c3string = $c3->value();
													
														if($c1->value() eq "" and $c3->value() eq "") { next; } #remove empty data
														
														my @first_splitted = split(' - ',$c1->value());
														my @splitted = split('/',first_splitted[0]);
														my $correct_data = @splitted[2] . '-' . @splitted[1] . '-' . @splitted[0];

														my $insert = $mango->db($mongo_database)->collection($mongo_collection)->insert({ "DATA_DIFUZARE" => clean_string($correct_data), "EMISIUNE" => "null", "MINUTE" => "null", "SECUNDE" => "null", "OPERA" => clean_string($c3string), "ARTIST" => "null", "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "STATUS" => "0", "TOTAL" => "0"});		
													}
												}												
									} 
