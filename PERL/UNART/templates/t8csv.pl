#
#
#	T8CSV template script (IMPORT V1)
# 	DO:
# 	1. Match template
# 	2. Parse file
# 	3. Import mongoDB
					        ...
					        $row += 1; 
					        	if($row > 9){ 	    
						            if ($csv->parse($_)) {
						                ...
						                my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => $splitted[0], "EMISIUNE" => $channel_founded[0], "MINUTE" => $splitted[2], "SECUNDE" => $splitted[3], "OPERA" => $splitted[5], "ARTIST" => $splitted[4], "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "STATUS" => "0"});
						            } else {
