								...
								my($data_sheet, $sheet_name); # set variables for parsing
									foreach my $data_sheet (@{$workbook->{Worksheet}}) {
										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (14 .. $data_sheet->{MaxRow}) {
													if($row != 14){
														...
														my $c5 = $data_sheet->get_cell($row, 1); next unless $c5; #minute
														my $c6 = $data_sheet->get_cell($row, 2); next unless $c6; #secunde
														...
														my $insert = $mango->db($mongo_database)->collection($mongo_collection)->insert({ "DATA_DIFUZARE" => clean_string($c1->value()), "EMISIUNE" => "null", "MINUTE" => clean_string($c5->value()), "SECUNDE" => clean_string($c6->value()), "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "STATUS" => "0", "TOTAL" => $total});
													}
												}										
									} 
