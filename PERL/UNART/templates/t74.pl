										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (8 .. $data_sheet->{MaxRow}) {
													if($row != 8){
														...
														if($c1->value() eq "" and $c3->value() eq "" and $c4->value() eq "") { next; } #remove empty data
														
														my @data = split(' ',$c1->value());
														my @second_data = split('/',@data[0]);
														
														my $correct_data = '20' . @second_data[2] . '-' . @second_data[1] . '-' . @second_data[0];
														
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => clean_string($correct_data), "EMISIUNE" => $channel_founded[0], "MINUTE" => "-", "SECUNDE" => "-", "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "STATUS" => "0", "TOTAL" => "null"});				
													}
												}
									} 
