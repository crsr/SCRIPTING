										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (10 .. $data_sheet->{MaxRow}) {
													if($row != 10){
														...
														if($c3->value() eq "" and $c4->value() eq "" and $c6->value() eq "") { next; } #remove empty data

														my $total = (($c5->value() * 60) + $c6->value()) * $c7->value();

														my $insert = $mango->db($mongo_database)->collection($mongo_collection)->insert({ "DATA_DIFUZARE" => "null", "EMISIUNE" => clean_string($c2->value()), "MINUTE" => clean_string($c5->value()), "SECUNDE" => clean_string($c6->value()), "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => clean_string($c7->value()), "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "STATUS" => "0", "TOTAL" => $total});	
													}
												}										
									} 
