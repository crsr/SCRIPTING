										...
										$sheet_name = undiacritic($data_sheet->{Name});
												...
												for my $row (0 .. $data_sheet->{MaxRow}) {
													if($row != 0){
														...
														my $c5 = $data_sheet->get_cell($row, 8); next unless $c5; #minute
														#my $c6 = $data_sheet->get_cell($row, 9); next unless $c6; #secunde
														...
														my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => clean_string($c1->value()), "EMISIUNE" => $channel_founded[0], "MINUTE" => clean_string($c5->value()), "SECUNDE" => "", "OPERA" => clean_string($c3string), "ARTIST" => "", "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "STATUS" => "0"});				
													}
												}									
									} 
