					        ...
					        $row += 1; 
					        	if($row > 8){ 	    
						            if ($csv->parse($_)) {
						                ...
						                my $insert = $mango->db('unart_parsing')->collection('parsed')->insert({ "DATA_DIFUZARE" => $splitted[1], "EMISIUNE" => $channel_founded[0], "MINUTE" => $splitted[3], "SECUNDE" => $splitted[4], "OPERA" => $splitted[5], "ARTIST" => $splitted[7], "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "STATUS" => "0"});
						            } else {
