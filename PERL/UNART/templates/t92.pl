										...
										$sheet_name = undiacritic($data_sheet->{Name});
											...
											for my $row (0 .. $data_sheet->{MaxRow}) {
													if($row != 0){
														...
														my $correct_data = $year_founded_in_condition_for_data[0] . '-' . $month_founded[0] . '-' . @data[0];

														my @minutes = split(':',$c5->value());
														my @seconds = split(':',$c6->value());
														my $total = ((@minutes[0] * 60) + @seconds[1]);

														my $insert = $mango->db($mongo_database)->collection($mongo_collection)->insert({ "DATA_DIFUZARE" => clean_string($correct_data), "EMISIUNE" => "null", "MINUTE" => @minutes[0], "SECUNDE" => @seconds[1], "OPERA" => clean_string($c3string), "ARTIST" => clean_string($c4->value()), "NR_DIFUZARI" => "-", "LUNA" => $month_founded[0], "AN" => $year_founded[0], "POST" => $channel_founded[0], "TEMPLATE" => $template, "STATUS" => "0", "TOTAL" => $total});
													}
												}								
									} 
